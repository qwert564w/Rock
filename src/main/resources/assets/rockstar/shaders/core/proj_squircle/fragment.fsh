...
void main() {
    vec2 TexCoord = gl_FragCoord.xy / RockstarUniforms.ScreenSize.xy;
    //TexCoord.y = 1.0 - TexCoord.y;

    vec2 center = RockstarUniforms.Size * 0.5;
    float distance = roundedBoxSDF(center - gl_FragCoord.xy, center - 1.0, RockstarUniforms.Radius, RockstarUniforms.CornerSmoothness);
